...
- **Scene assembly (current)**: `Tools/Coop Setup/Multiplayer Flow/Run All (1-4)` (or the numbered sub-steps individually; `Assets/Editor/GameFlowSceneSetup.cs`) rebuilds `Bootstrap.unity`/`Lobby.unity`/`Stage01_Gatekeeper.unity` from scratch and re-registers them in `EditorBuildSettings.scenes` — headless via `-executeMethod GameFlowSceneSetup.RunAll`. **This is destructive-by-overwrite**: each step calls `EditorSceneManager.NewScene(EmptyScene, Single)`, so re-running it discards any manual Inspector tweaks made directly in those scene files since the last run — treat the scenes as generated output, hand-edit only for throwaway testing.
...
### Stage design docs / stage-select UI prototype (currently unreconciled with the flow above)

`Docs/Stages/` holds design docs for 4 planned missions (4-6 players), each built around one recurring Pico Park co-op mechanic pillar — see `Docs/Stages/README.md` for the index and `Docs/Stages/bot-coordination.md` for how bots are meant to auto-clear each stage (role assignment via a server-written `NetworkVariable`, new `BotMode`s like `HoldButton`/`PushTarget`/`CarryRelay`/`GroupAdvance`). Only stage 1 ("Gatekeeper") has an actual scene (`Stage01_Gatekeeper.unity`, built by `GameFlowSceneSetup`); the other 3 designs have no gameplay implementation yet, nor do the shared mechanic scripts the docs call for (`PushableBlockNGO`, `CarryableKeyNGO`, `SeesawPlatformNGO`, `RisingHazardNGO`, a bot role-assignment director).

- **`Assets/Scripts/Stage/StageDefinition.cs`** / **`StageCatalog.cs`** — `ScriptableObject`s holding catalog metadata (title/theme/min-max players) per stage, mirroring the `Docs/Stages/*.md` docs 1:1. Populated under `Assets/StageData/` via `Tools/Coop Setup/7. Create Stage Catalog (4 Missions)` (legacy `NetworkSetupMenu.cs`).
- **`Assets/Scripts/UI/WorldMapUI.cs`** / **`StageNodeButton.cs`** / **`StageIntroUI.cs`** — a manual "pick a stage from a map" UI (Pico Park 2 WORLD-mode-style), built into `Assets/scense/main.unity`'s `WorldMapCanvas` via `Tools/Coop Setup/8`.
- **This diverges from `GameFlowManager`'s actual design**: `GameFlowManager` auto-advances through `stageSceneNames` sequentially on a lobby countdown — there's no player-driven stage selection in the live flow, and `WorldMapUI`/`StageIntroUI` aren't referenced by `GameFlowSceneSetup`, `GameFlowManager`, or any Bootstrap/Lobby/Stage scene. Whoever picks this back up needs to decide whether stage selection becomes a real feature (and if so, wire the map screen into `GameFlowManager` in place of/alongside the auto-countdown) or whether the catalog/map UI should be dropped in favor of the simpler sequential flow — don't assume either is already decided.

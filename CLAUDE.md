...
- **`Assets/Editor/GameFlowSceneSetup.cs`** (`Tools/Coop Setup/Multiplayer Flow/*`) — builds the *current* `Bootstrap`/`Lobby`/`Stage01_Gatekeeper` scenes and owns `EditorBuildSettings.scenes`. Its own `BuildConnectUI()` is a **plain, unthemed** Connect UI (flat `Image.color`, `LegacyRuntime.ttf`) — it does *not* call into `UITheme.cs` or the procedural rounded-sprite/font helpers below, unlike the legacy version. If the Bootstrap scene's connect button looks generic, that's why; wire it to `UITheme.cs` if that's ever worth doing.
- **`Assets/Editor/NetworkSetupMenu.cs`** (`Tools/Coop Setup/1-8`) — the legacy single-scene prototype (targets `Assets/scense/main.unity`, no longer built). Still the sole owner of `Assets/Prefabs/Player.prefab` generation (step 2) and of the procedural-sprite/UI helper methods (`GetOrCreateRoundedRectSprite`, `GetOrCreatePlayerCharacterSprite`, `CreateOrGetLabel`) that *do* apply the full UI Style Guide — those are reused by, e.g., its own themed Connect UI and the `WorldMapUI`/`StageIntroUI` stage-select prototype (see "Stage system" below).
...
- **Known gap**: `NetworkSetupMenu.cs`'s Connect UI (legacy scene) applies these fonts/colors correctly. `GameFlowSceneSetup.cs`'s Connect UI (the one actually live in `Bootstrap.unity`) does **not** — it's a separate, plainer implementation (`LegacyRuntime.ttf`, flat `Image.color`). See "Editor automation" above.

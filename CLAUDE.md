...
### Stage system (map / stage-intro UI, design docs)

`Docs/Stages/` holds the design docs for 4 planned missions (4-6 players), each built around one recurring Pico Park co-op mechanic pillar — see `Docs/Stages/README.md` for the index and `Docs/Stages/bot-coordination.md` for how bots are meant to auto-clear each stage (role assignment via a server-written `NetworkVariable`, new `BotMode`s like `HoldButton`/`PushTarget`/`CarryRelay`/`GroupAdvance`). **Only the catalog data and selection UI are implemented so far — the actual gameplay mechanics each doc describes (`PushableBlockNGO`, `CarryableKeyNGO`, `SeesawPlatformNGO`, `RisingHazardNGO`, the bot role-assignment director) are design-only, not yet built.**

- **`Assets/Scripts/Stage/StageDefinition.cs`** — a `ScriptableObject` holding one stage's catalog metadata (`titleEn`/`subtitleKr`/`descriptionKr`, `StageTheme`, min/max players). Mirrors a `Docs/Stages/*.md` doc 1:1 — if one changes, update the other.
- **`Assets/Scripts/Stage/StageCatalog.cs`** — an ordered `StageDefinition[]`. `Tools/Coop Setup/7. Create Stage Catalog (4 Missions)` creates/updates the 4 mission assets under `Assets/StageData/` from hardcoded data matching the docs (create-or-update, safe to re-run after editing the docs and updating the menu method's literals to match).
- **`Assets/Scripts/UI/WorldMapUI.cs`** / **`StageNodeButton.cs`** / **`StageIntroUI.cs`** — the "게임내 맵 화면" (stage-select nodes, Pico Park 2 WORLD-mode-style) and "게임 스테이지 화면" (per-stage detail card: title/theme/description/player count + Start/Back). `Tools/Coop Setup/8. Create World Map + Stage Intro UI` builds both into a `WorldMapCanvas` (idempotent-update like Connect UI). `StageIntroPanel` starts inactive; clicking a node hides the map and shows the intro card, Back reverses it.
- **Known gap**: `StageIntroUI.OnStartPressed()` only logs — there's no wiring yet from "스테이지 시작" to actually loading that stage's gameplay (this project is still single-scene). `WorldMapCanvas` also isn't hooked into `NetworkBootstrapper`'s connect flow (i.e. Connect UI → World Map isn't automatic yet). Both are next steps once the gameplay mechanics themselves exist.

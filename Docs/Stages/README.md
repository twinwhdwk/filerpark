...
| 3 | [열쇠 릴레이 (Key Relay)](03-key-relay.md) | 운반 & 균형 | 구현 완료(원안에서 단순화됨), 5봇 GCP 실서버 테스트로 검증됨 |
| 4 | [탈출 카운트다운 (Escape Countdown)](04-escape-countdown.md) | 공동 생존 | 구현 완료, 5봇 GCP 실서버 테스트로 검증됨 |
| 5 | 쌍둥이 문지기 (Twin Gatekeeper) | 동시 입력 (심화) | 구현 완료. Stage 1과 같은 컴포넌트(`CoopButtonNGO`/`CoopDoorNGO`/`GoalZoneNGO`)를 재사용해 버튼을 하나가 아니라 둘로 늘린 변형 — 별도 설계 문서 없이 `CLAUDE.md`의 "Stage 5" 절에 직접 기술됨. 2봇 헤드리스 클리어 검증 완료(`Assets/Editor/Stage05BotVerify.cs`); 4~5봇 GCP 실서버 연속 순환 테스트는 아직 안 함(Stage 1-4 수준의 검증은 아직 없음).

전 스테이지 5봇 연속 순환(Stage1→2→3→4→1…) 실서버 테스트가 ~9분간 매 사이클 클리어를 반복하는 것으로 확인됨 (커밋 `2b96135`, Stage 5 추가 이전 기준).
